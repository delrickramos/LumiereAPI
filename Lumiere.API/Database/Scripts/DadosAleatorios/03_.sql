...
INSERT INTO Ingressos (PrecoFinal, ExpiraEm, Status, SessaoId, AssentoId, TipoIngressoId)
SELECT
    CAST(s.PrecoBase * (1 - (t.DescontoPercentual / 100.0)) AS decimal(18,2)) AS PrecoFinal,
    CASE st.StatusInt
        WHEN 0 THEN DATEADD(MINUTE, 30, SYSDATETIMEOFFSET())  -- Reservado expira em 30 min
        ELSE SYSDATETIMEOFFSET()                              -- Cancelado/Confirmado: valor válido
    END AS ExpiraEm,
